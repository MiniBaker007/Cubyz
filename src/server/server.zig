...
pub const User = struct { // MARK: User
	...
	pub fn regenerateHealth(self: *User) void {
		...
		if(self.lastHealthRegenTime.durationTo(healthRegenTime).toSeconds() > 5 and self.player.energy > self.player.maxEnergy - 1 and self.player.health < self.player.maxHealth) {
